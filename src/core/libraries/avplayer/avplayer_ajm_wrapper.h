...
class AjmAacDecoder {
public:
    AjmAacDecoder(u32 context_id, u32 channels) : m_context_id(context_id), m_batch_buffer(4096) {
    ...
    void Init(u32 sampling_freq_type) {
        ...
        void* current = m_batch_buffer.data();
        current = sceAjmBatchJobControlBufferRa(current, m_instance_id, flags.raw, &init_params,
                                                sizeof(init_params), &result, sizeof(result),
                                                __builtin_return_address(0));
        AjmBatchError error{};
        u32 batch_id{};
        sceAjmBatchStartBuffer(m_context_id, m_batch_buffer.data(),
                               reinterpret_cast<u8*>(current) - m_batch_buffer.data(), 40, &error,
                               &batch_id);
    ...
    u32 Decode(std::span<const u8> input, std::span<u8> output) {
        ...
        struct Sideband {
            ...
            AjmSidebandMFrame mframe;
        } sideband;

        AjmJobFlags flags{
            .version = 1,
            .run_flags = AjmJobRunFlags::MultipleFrames,
            .sideband_flags = AjmJobSidebandFlags::Stream,
        };
        void* current = m_batch_buffer.data();
        current = sceAjmBatchJobRunBufferRa(
            current, m_instance_id, flags.raw, const_cast<u8*>(input.data()), input.size(),
            output.data(), output.size(), &sideband, sizeof(sideband), __builtin_return_address(0));
        AjmBatchError error{};
        u32 batch_id{};
        sceAjmBatchStartBuffer(m_context_id, m_batch_buffer.data(),
                               reinterpret_cast<u8*>(current) - m_batch_buffer.data(), 40, &error,
                               &batch_id);
        sceAjmBatchWait(m_context_id, batch_id, -1, &error);
        return sideband.stream.output_written;
    }
